...
const alert =await this.alertController.create({
  ...
  buttons: [{
    ...
    text: '<h1 class="c-no-margin winner-alert-title">Congratulations!</h1>',
    handler: () =>{
...
billdesk(){
 ...
 this.common.createOrder(data,localStorage.getItem('token')).subscribe(res=> {
  ...
  var flow_config = {
   ...
   this.common.launchSdk(res['merchantId'],res['bdOrderId'],res['authToken'],true,"http://localhost/dashboard",3).subscribe(res=>{
    console.log(res)
   })
 })
}
